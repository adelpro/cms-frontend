...
import { cn, convertListAssetToAsset } from '@/lib/utils';
    ...
    const loadAssets = async () => {
      try {
        ...
        if (selectedCategories.length > 0) {
          ...
          const mappedCategories = selectedCategories.map(cat => categoryMap[cat]).filter(Boolean); // Remove undefined values
              ...
              <>
                <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mb-8 auto-rows-fr'>
                  {displayAssets.map(asset => (
                    <Card
                      key={asset.id}
                      className='shadow-[0px_4px_6px_-4px_rgba(0,0,0,0.1),0px_10px_15px_-3px_rgba(0,0,0,0.1)] hover:shadow-[0px_8px_10px_-6px_rgba(0,0,0,0.1),0px_16px_20px_-5px_rgba(0,0,0,0.1)] transition-shadow h-full'
                    >
                      <CardHeader className='flex items-center justify-between gap-2'>
                        ...
                        <Badge
                          variant='outline'
                          className={cn(
                            'w-fit max-w-[130px] text-xs border border-neutral-200 truncate'
                          )}
                        >
                          {asset.license}
                        </Badge>
