...
export function PublisherProfile({ publisherId, locale }: PublisherProfileProps) {
  ...
  const licenses = [
    ...
    { id: 'CC-BY', name: 'CC BY', label: t('licenses.ccBy'), color: 'green' },
    {
      id: 'CC-BY-SA',
      name: 'CC BY-SA',
      label: t('licenses.ccBySa'),
      color: 'yellow',
    },
    {
      id: 'CC-BY-ND',
      name: 'CC BY-ND',
      label: t('licenses.ccByNd'),
      color: 'yellow',
    },
    {
      id: 'CC-BY-NC',
      name: 'CC BY-NC',
      label: t('licenses.ccByNc'),
      color: 'yellow',
    },
    {
      id: 'CC-BY-NC-SA',
      name: 'CC BY-NC-SA',
      label: t('licenses.ccByNcSa'),
      color: 'red',
    },
    {
      id: 'CC-BY-NC-ND',
      name: 'CC BY-NC-ND',
      label: t('licenses.ccByNcNd'),
      color: 'red',
    },
  ];
            ...
            >
              {categories.map((category) => (
                <div key={category.key} className={cn('flex items-center', spacing.gapSm)}>
                  ...
                  />
                  <label htmlFor={`category-${category.key}`} className="cursor-pointer text-sm">
                    {category.label}
                  </label>
                </div>
              ))}
                ...
                <div key={license.id} className={cn('flex flex-col', spacing.gapSm)}>
                  ...
                  <div>
                    <Badge
                      variant="outline"
                      className={cn(
                        'h-6 w-fit border-0 px-2 text-xs text-white',
                        (license.id === 'CC-BY' || license.id === 'CC0') && 'bg-emerald-500',
                        (license.id === 'CC-BY-SA' ||
                          license.id === 'CC-BY-ND' ||
                          license.id === 'CC-BY-NC') &&
                          'bg-amber-500',
                        (license.id === 'CC-BY-NC-SA' || license.id === 'CC-BY-NC-ND') &&
                          'bg-red-500',
                      )}
                    >
              ...
              <div className="flex flex-col gap-6 md:flex-row">
                <div className="flex-shrink-0">
                  <Avatar className="h-24 w-24">
                    <AvatarImage
                      src={`${env.NEXT_PUBLIC_BACKEND_URL}${publisher.icon_url}`}
                      alt={publisher.name}
                    />
              ...
              <>
                <div className="mb-8 grid auto-rows-fr grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
                  {displayAssets.map((asset) => (
                    <Card
                      key={asset.id}
                      className="h-full shadow-[0px_4px_6px_-4px_rgba(0,0,0,0.1),0px_10px_15px_-3px_rgba(0,0,0,0.1)] transition-shadow hover:shadow-[0px_8px_10px_-6px_rgba(0,0,0,0.1),0px_16px_20px_-5px_rgba(0,0,0,0.1)]"
                    >
                      <CardHeader className="flex items-center justify-between gap-2">
                        ...
                        <Badge
                          variant="outline"
                          className={cn(
                            'w-fit max-w-[130px] truncate border border-neutral-200 text-xs',
                          )}
                        >
                          {asset.license}
                        </Badge>
                          ...
                          <div className="flex flex-1 flex-col justify-between gap-14">
                            ...
                            <div className="space-y-2">
                              <div className="text-muted-foreground flex flex-col text-sm">
                                <span className="font-medium">{t('ui.publisherLabel')}</span>{' '}
                                {asset.publisher}
                              </div>
                            </div>
                          </div>
